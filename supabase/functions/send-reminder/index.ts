    ...
    const html = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
        ...
        <div style="border: 1px solid #e0e0e0; border-top: none; padding: 24px; border-radius: 0 0 8px 8px;">
          <h2 style="margin-top: 0; color: #2d5a27;">Payment Reminder — ${invoice.invoice_number}</h2>
          ${invoiceDate ? `<p style="margin: 4px 0; color: #888; font-size: 0.9rem;">Date: ${invoiceDate}</p>` : ''}

          <p>Hi ${customer?.name || 'there'},</p>
          <p>Just a friendly reminder that invoice <strong>${invoice.invoice_number}</strong> is still outstanding.${dueDate ? ` Payment was due by <strong>${dueDate}</strong>.` : ''}</p>
          <p>Here's a summary of the invoice:</p>
          ...
          <p style="color: #888; font-size: 0.85rem;">If you've already made payment, please disregard this reminder.</p>
